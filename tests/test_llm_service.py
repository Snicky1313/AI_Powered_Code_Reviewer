...
def test_health_check():
    ...
    try:
        ...
        print("    PYTHONPATH=src python -m ai_code_reviewer.analyzers.llm_feedback")
        return False
...
"""
    analysis_results = {
        "syntax": {"ok": True, "findings": [], "filename": "good_code.py"},
        "style": {
            "success": True, "style_score": 95.0, "violations": [],
            "summary": {"total_violations": 0, "errors": 0, "warnings": 0, "grade": "A"}
        }
    }
    try:
        response = requests.post(
            f"{LLM_SERVICE_URL}/feedback",
            json={'code': good_code, 'analysis_results': analysis_results, 'user_id': 'test_user', 'submission_id': 'test_good_001'},
            timeout=30
        )
...
"""
    analysis_results = {
        "syntax": {
            "ok": False,
            "findings": [
                {"id": "SYN001-4-9-1", "message": "invalid syntax", "location": {"start": {"line": 4, "column": 9}}, "suggestion": "Add a colon (:) at the end of the statement header."},
                {"id": "SYN001-5-11-2", "message": "EOL while scanning string literal", "location": {"start": {"line": 5, "column": 11}}, "suggestion": "Close the string with matching quotes."}
            ],
            "filename": "bad_code.py"
        },
        "style": {
            "success": True, "style_score": 65.0,
            "violations": [
                {"line": 1, "column": 8, "code": "E231", "text": "missing whitespace after ','", "severity": "warning"},
                {"line": 2, "column": 1, "code": "E101", "text": "Mixed tabs and spaces", "severity": "error"},
                {"line": 2, "column": 13, "code": "W291", "text": "Trailing whitespace", "severity": "warning"}
            ],
            "summary": {"total_violations": 3, "errors": 1, "warnings": 2, "grade": "D"}
        },
        "security": {"ok": True, "findings": [], "filename": "bad_code.py"}
    }
    try:
        response = requests.post(
            f"{LLM_SERVICE_URL}/feedback",
            json={'code': bad_code, 'analysis_results': analysis_results, 'user_id': 'test_user', 'submission_id': 'test_bad_001'},
            timeout=30
        )
        result = response.json()
        print_result(result)
...
"""
    analysis_results = {
        "syntax": {"ok": True, "findings": [], "filename": "insecure_code.py"},
        "style": {"success": True, "style_score": 88.0, "violations": [], "summary": {"total_violations": 0, "errors": 0, "warnings": 0, "grade": "B"}},
        "security": {
            "ok": False,
            "findings": [
                {"message": "subprocess call with shell=True identified, security issue.", "severity": "HIGH", "location": {"start": {"line": 5, "col": 1}}, "suggestion": "Avoid using shell=True. Use subprocess.run() with a list of args."},
                {"message": "Use of insecure MD5 hash function.", "severity": "MEDIUM", "location": {"start": {"line": 8, "col": 1}}, "suggestion": "Replace MD5 with SHA-256 or stronger."}
            ],
    ...
    try:
        response = requests.post(
            f"{LLM_SERVICE_URL}/feedback",
            json={'code': insecure_code, 'analysis_results': analysis_results, 'user_id': 'test_user', 'submission_id': 'test_security_001'},
            timeout=30
        )
        result = response.json()
        print_result(result)
...
"""
    try:
        response = requests.post(
            ...
            json={'code': test_code, 'user_id': 'test_user', 'language': 'python', 'analysis_types': ['syntax'], 'include_llm_feedback': True},
            timeout=30
        )
        if response.status_code == 200:
            result = response.json()
        ...
        print("    PYTHONPATH=src python -m ai_code_reviewer.main")
        return False
    except Exception as e:
...
def main():
    ...
    if not has_api_key:
        ...
        print("Skipping tests that require OpenAI API…")
        return
    time.sleep(1); results.append(("API Connectivity", test_api_connectivity()))
    time.sleep(1); results.append(("Good Code Feedback", test_good_code_feedback()))
    time.sleep(1); results.append(("Bad Code Feedback", test_bad_code_feedback()))
    time.sleep(1); results.append(("Security Issues Feedback", test_security_issues_feedback()))
    time.sleep(1); results.append(("End-to-End Integration", test_end_to_end()))
    print_header("Test Summary")
    passed = sum(1 for _, r in results if r); total = len(results)
    for name, r in results: print(f"{'✓ PASS' if r else '✗ FAIL'} - {name}")
