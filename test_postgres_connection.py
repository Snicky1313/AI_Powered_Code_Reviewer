...
def test_postgres_connection():
    ...
    try:
        # Test connection as postgres superuser
        ...
        print("2. Wrong password (you provided: Och13ng)")
        print("3. PostgreSQL is not listening on localhost:5432")
        print("4. User 'postgres' doesn't exist")
        return False
...
def setup_logging_database():
    ...
    try:
        ...
        with connection.cursor() as cursor:
            ...
            # Create indexes for review_sessions table
            ...
            # Create indexes for log_events table
...
def test_logging_user_connection():
    ...
    try:
        ...
        with connection.cursor() as cursor:
            ...
            print(f"✅ Can read users table: {user_count['count']} records")
            
            cursor.execute("SELECT COUNT(*) as count FROM review_sessions")
            session_count = cursor.fetchone()
            print(f"✅ Can read review_sessions table: {session_count['count']} records")
            
            cursor.execute("SELECT COUNT(*) as count FROM log_events")
            event_count = cursor.fetchone()
            print(f"✅ Can read log_events table: {event_count['count']} records")
...
def main():
    ...
    if not test_postgres_connection():
        print("\n❌ Cannot proceed without PostgreSQL connection. Please check your PostgreSQL installation and credentials.")
        return False
    ...
    # Step 3: Test logging user connection
    if not test_logging_user_connection():
        print("\n❌ Logging user connection failed. Please check the error messages above.")
        return False
    
    print("\n🎉 All tests passed! Your database is ready for the logging service.")
    print("\nNext steps:")
    print("1. Run: python consumer.py")
    print("2. In another terminal, run: python producer.py")
    print("3. Test with: python test_logging_service.py")
