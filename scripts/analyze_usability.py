...
# ---------- Portable paths ----------
# This file lives in repo_root/scripts/, so parents[1] is the repo root.
ROOT = Path(__file__).resolve().parents[1]
DATA = ROOT / "data"
WEB = ROOT / "web_demo"  # reserved if you ever need web assets


def _resolve_csv(arg: str | None) -> Path:
    """
    Resolve a CSV path with sensible defaults:
    - None -> data/survey_results.csv
    - Absolute path -> returned as-is
    - Relative path -> interpreted relative to repo root
    """
    if arg is None:
        return DATA / "survey_results.csv"
    p = Path(arg)
...
def analyze_survey_results(csv_file: str | None = None) -> None:
    """
    Analyze SUS survey results from a CSV file and print comprehensive results.

    Args:
        csv_file: Optional path to the CSV file. If None, defaults to data/survey_results.csv
    """
    path = _resolve_csv(csv_file)
    try:
        # Read the CSV file
        ...
        feedback_df = df[
            df["qualitative_feedback"].notna()
            & (df["qualitative_feedback"].astype(str).str.strip() != "")
        ]
        if feedback_df.empty:
            ...
            for _, row in feedback_df.iterrows():
                ...
                feedback = str(row["qualitative_feedback"]).strip()
        ...
        print("Tip: put your CSV at data/survey_results.csv or pass a custom path.")
...
def create_sample_data() -> None:
    """
    Create a sample data/survey_results.csv file for testing purposes.
    """
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == "--create-sample":
            ...
            analyze_survey_results(sys.argv[1])  # custom path
    else:
        analyze_survey_results(None)  # default: data/survey_results.csv
